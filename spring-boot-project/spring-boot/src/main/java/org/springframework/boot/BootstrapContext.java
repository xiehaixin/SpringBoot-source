 ...
 *
 * 一个简单的引导上下文，在启动和环境后处理期间可用，直到ApplicationContext准备好为止。
 *
 * <p>
 ...
 *
 * 提供对单例的延迟访问，这些单例的创建成本可能很高，或者需要在ApplicationContext可用之前共享。
 *
 * @author Phillip Webb
 * @since 2.4.0
 */
public interface BootstrapContext {
	 ...
	 *
	 * 如果类型已注册，则从上下文返回一个实例。 如果之前没有访问过实例，就会创建它。
	 *
	 ...
	 * @param <T> the instance type
	 * @param type the instance type
	 * @param other the instance to use if the type has not been registered 如果类型尚未注册，则使用的实例
	 * @return the instance
	 */
	 ...
	 * @param <T> the instance type
	 * @param type the instance type
	 * @param other a supplier for the instance to use if the type has not been registered 如果类型尚未注册，则使用实例的供应商
	 * @return the instance
	 */
	 ...
	 *
	 * @param <T> the instance type
	 * @param <X> the exception to throw if the type is not registered 如果类型未注册，则抛出的异常
	 * @param type the instance type
